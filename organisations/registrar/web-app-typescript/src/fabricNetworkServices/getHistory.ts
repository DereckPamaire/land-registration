...
async function connectToGetLandTitleHistory(landId: string): Promise<any> {
...
async function getLandTitleById(contract: Contract, landId: string): Promise<any>{
	console.log(`\n--> Query the ledger: For History, ${landId}`);
	let result;
	try
	{
		// use evaluate to query the ledger
		const resultBytes = await contract.evaluateTransaction('GetLandTitleHistory', landId );
...
export { connectToGetLandTitleHistory };
